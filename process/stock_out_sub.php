  ...
  if ($count == 1) {
    $Sql = "INSERT INTO stock_out
      ( DocNo,DocDate,DepCode,RefDocNo,
		...
		stock_out.Modify_Code,stock_out.Modify_Date )
      VALUES
...
function ShowDocument($conn, $DATA)
{
  ...
  // mysqli_query($conn,$Sql);
    $Sql = "SELECT
      ...
      employee.LastName,TIME(stock_out.Modify_Date) AS xTime,stock_out.IsStatus
    FROM stock_out_detail
    INNER JOIN item ON item.ItemCode = stock_out_detail.ItemCode
    INNER JOIN stock_out ON stock_out.DocNo = stock_out_detail.DocNo
    INNER JOIN department ON department.DepCode = stock_out.DepCode
    INNER JOIN department_sub ON department_sub.DepSubCode = stock_out_detail.DepSubCode
    ...
    if ($selecta == 0) {
      ...
      $Sql .= " AND hospital.HptCode = $HptCode AND stock_out_detail.DepSubCode = $deptCode_sub";
    }
  $Sql .= " ORDER BY stock_out_detail.Id DESC LIMIT 500";
  $return['sql'] = $Sql;
  $meQuery = mysqli_query($conn, $Sql);
  while ($Result = mysqli_fetch_assoc($meQuery)) {
    $return[$count]['HptName']   = $Result['HptName'];
...
function getImport($conn, $DATA)
{
  ...
  for ($i = 0; $i < $max; $i++) {
    ...
    if ($chkUpdate == 0) {
      if ($Sel == 1) {
        $Sql = "INSERT INTO stock_out_detail
            (DocNo,ItemCode,UnitCode,Qty,IsCancel)
            VALUES
            ('$DocNo','$ItemCode',$iunit2,$iqty2,0)";
        mysqli_query($conn, $Sql);
      } else {
        $Sql = "INSERT INTO stock_out_detail_sub
            (DocNo,ItemCode)
            VALUES
            ('$DocNo','$ItemCode')";
        mysqli_query($conn, $Sql);
  ...
  if ($Sel == 2) {
    ...
    $Sql = "SELECT ItemCode,Qty FROM stock_out_detail WHERE stock_out_detail.DocNo = '$DocNo'";
    $meQuery = mysqli_query($conn, $Sql);
    while ($Result = mysqli_fetch_assoc($meQuery)) {
...
function findDoc($conn, $DATA)
{
  ...
  $DepSubCode = $DATA['DepSubCode'];
    ...
    $Sql = "SELECT
        ...
        INNER JOIN users ON stock_out.Modify_Code = users.ID
        INNER JOIN employee ON users.EmpCode = employee.EmpCode
        WHERE hospital.HptCode = $HptCode AND department.DepCode = $DepCode AND stock_out_detail.DepSubCode = $DepSubCode";
        $return['Sql'] = $Sql;
    // if ($selecta == 0) {
    //   $Sql .= "WHERE stock_out.DepCode = $deptCode AND stock_out.DocNo LIKE '%$DocNo%'";
    // }
