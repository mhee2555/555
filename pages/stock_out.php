...
<head>
  ...
  <script type="text/javascript">
    ...
    function Save_dis(){
      ...
      var iqty = $('#iqty_dis').val();
      var Docnox = $('#dis_DocNo').val();
      var deptCodex = $('#DepCode_x').data('value');
      var DepSubCodex = $('#DepSubCode_x').data('value');
      var ItemCodex = $('#item_x').data('value');
      var DepSubCodeFromx = $('#DepSubCodeFrom_x').data('value');
      var deptCodeFromx = $('#DepCodeFrom_x').data('value');
      var data = {
        'STATUS': 'save_dis',
        'Docnox': Docnox,
        'deptCodex': deptCodex,
        ...
        'ItemCodex': ItemCodex,
        'iqty': iqty,
        'DepSubCodeFromx': DepSubCodeFromx,
        'deptCodeFromx': deptCodeFromx
        ...
        success: function (result) {
          ...
          if (temp["status"] == 'success') {
            if (temp["form"] == 'OnLoadPage') {
              ...
              $('#dis_DocDate').val(temp['DocDate']);
              $('#dis_DocNo').val(temp['DocNo']);
              $('#dis_Recorder').val(temp['Emp']);
              $('#dis_Timerec').val(temp['xTime']);
              $('#dis_department').val(temp['DepFrom']);
              $('#department_sub').val(temp['DepSubFrom']);
              $("#table_disburse tbody").empty();
              $("#table_disburse tbody").empty();
              
              for (var i = 0; i < (Object.keys(temp).length - 2); i++) {
                var Qty = "<div class='row' style='margin-left:54px;'><button class='btn btn-danger'  style='height:40px;width:32px;' onclick='subtractnum2(\"" + i +"\")'>-</button><input type='number' class='form-control text-center' style='width:155px;font-size:25px;margin:2px;' id='iqty_dis' value='"+temp[i]['Qty']+"' min='1' data-value='"+temp[i]['Qty']+"' max='"+temp[i]['Qty']+"' onkeyup='if(this.value > "+temp[i]['Qty']+"){this.value="+temp[i]['Qty']+"}else if(this.value<=0){this.value=1}'><button class='btn btn-success' style='height:40px;width:32px;' onclick='addnum2(\"" + i +"\")'>+</button></div>";
                ...
                StrTr = "<tr id='tr" + temp[i]['ItemCode'] + "'>" +
                  ...
                  "<td style='width: 12%;text-align: center;'>" + temp[i]['Qty'] + "</td>" +
                  ...
                  "<td style='width: 25%;text-align:center;'>"+Qty+"</td>" +
                  "</tr>";
                ...
                if(temp['IsStatus']==1){
                  ...
                  $('#iqty_dis').attr('disabled', true);
                  $('#btn_confirm').text('อนุมัติแล้ว');
                }else{
                  $('#btn_confirm').attr('disabled', false);
                  $('#iqty_dis').attr('disabled', false);
                  $('#btn_confirm').text('อนุมัติ');
    ...
    function addnum2(Id) {
      var rowid = $('#iqty_dis').val();
      var qtyMax = $('#iqty_dis').data('value');
      var add = parseInt($('#iqty_dis').val()) + 1;
      if  (add > qtyMax) {
        $('#iqty_dis').val(qtyMax);
      }else{
        $('#iqty_dis').val(add);
    ...
    function subtractnum2(Id) {
      var rowid = $('#iqty_dis').val();
      var sub = parseInt($('#iqty_dis').val()) - 1;
      if (sub <= 0) {
        $('#iqty_dis').val(1);
      }else{
        $('#iqty_dis').val(sub);
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
              <li class="nav-item">
                <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home"
                  aria-selected="true"><?php echo $array['stockout'][$language]; ?></a>
              </li>
              <li class="nav-item">
              ...
              <li class="nav-item">
                <a class="nav-link" id="profile-tab" data-toggle="tab" href="#disburse" role="tab" aria-controls="profile" aria-selected="false"><?php echo $array['disburse'][$language]; ?></a>
              </li>
            </ul>
            ...
            <div class="tab-content" id="myTabContent">
                        ...
                        <div class="row" style="margin-top:5px">
                          ...
                          <div style="width:220px;visibility: hidden">
                            <input class='form-control' style="font-size:20px;width:220px;height:40px;padding-top:6px;"
                              id='RefDocNo' placeholder="<?php echo $array['refdocno'][$language]; ?>" OnBlur='UpdateRefDocNo()'>
                          </div>
                        </div>
                        <div class="row" style="margin-top:5px;">
                          <div style="margin-left:20px;width:100px;">
                      ...
                      <div class="card-body" style="padding:0px; margin-top:10px;">
                          ...
                          <div class="col-md-1">
                            <div class="row" style="margin-left:2px;">
                              <div class="row" style="margin-left:20px;">
                                <button style="width:105px" ; type="button" class="btn btn-info"
                                  onclick="CreateDocument()"
                                  id="bCreate"><?php echo $array['createdocno'][$language]; ?></button>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="row" style="margin-top:4px;">
                          <div class="col-md-1">
                            <div class="row" style="margin-left:2px;">
                              <div class="row" style="margin-left:20px;">
                                <button onclick="OpenDialogItem()" type="button" style="width:105px"
                                  class="btn btn-warning"
                                  id="bImport"><?php echo $array['import'][$language]; ?></button>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                <div class="row" style="margin-top:10px;">
                  ...
                  <div class="col-md-6">
                    <div class="row" style="margin-left:2px;">
                      <input type="text" class="form-control" style="font-size:24px;width:50%;" name="searchdocument"
                        id="searchdocument" placeholder="<?php echo $array['searchplace'][$language]; ?>">
              ...
              <!-- search document -->
              <div class="tab-pane fade" id="disburse" role="tabpanel">
                <div class="row" style="margin-top:10px;">
                  <div class="col-md-4">
                    <div class="row" style="font-size:24px;margin-left:2px;">
                  ...
                  <div class="col-md-2">
                    <button type="button" style="margin-left:90px;" class="btn btn-warning" name="button" onclick="SelectDocument_dis();"><?php echo $array['show'][$language]; ?></button>
                  </div>
                </div>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <!-- tag column 1 -->
                    <table style="margin-top:10px;" class="table table-fixed table-condensed table-striped"
                      id="TableDocument_dis" width="100%" cellspacing="0" role="grid">
                      <thead id="theadsum" style="font-size:24px;">
                        <tr role="row">
                          <th style='width: 3%;'>&nbsp;</th>
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <div class="row" style="margin-top:5px">
                      ...
                      <div style="width:220px;">
                        <input type="text" class="form-control" style="font-size:24px;width:220px;" disabled id="dis_DocDate" placeholder="<?php echo $array['docdate'][$language]; ?>">
                      </div>
                      <div style="margin-left:30px;width:120px;">
                        <label><?php echo $array['docno'][$language]; ?></label>
                      </div>
                      <div style="width:220px;">
                        <input type="text" class="form-control" style="font-size:24px;width:220px;" disabled id="dis_DocNo" placeholder="<?php echo $array['docno'][$language]; ?>">
                      </div>
                      ...
                      <div style="width:220px;">
                        <input class='form-control' style="font-size:20px;width:220px;height:40px;padding-top:6px;" class="form-control" id="dis_department" disabled>
                      </div>
                    </div>
                    <div class="row" style="margin-top:5px;">
                      <div style="margin-left:20px;width:100px;">
                      ...
                      <div style="width:220px;">
                        <input type="text" class="form-control" style="font-size:24px;width:220px;" id="dis_Recorder" disabled  placeholder="<?php echo $array['employee'][$language]; ?>">
                      </div>
                      <div style="margin-left:30px;width:120px;">
                        <label><?php echo $array['time'][$language]; ?></label>
                      </div>
                      <div style="width:220px;">
                        <input type="text" class="form-control" style="font-size:24px;width:220px;" id="dis_Timerec" disabled placeholder="<?php echo $array['time'][$language]; ?>">
                      </div>
                      <div style="margin-left:30px;width:120px;">
                        <label><?php echo $array['department_sub'][$language]; ?></label>
                      </div>
                      <div style="width:220px;">
                        <input class='form-control' style="font-size:20px;width:220px;height:40px;padding-top:6px;" class="form-control" id="department_sub" disabled>
                      </div>
                    </div>
                    <hr>
                    <div class="row">
                        ...
                        <table class="table table-fixed table-condensed table-striped" id="table_disburse"  cellspacing="0" role="grid" style="font-size:24px;font-family: 'THSarabunNew'">
                          <thead style="font-size:24px;">
                            <tr role="row">
                              ...
                              <th style='width: 12%;'><center><?php echo $array['disburse'][$language]; ?></center></th>
                              <th style='width: 25%;'><center><?php echo $array['numprice'][$language]; ?></center></th>
                            </tr>
                          </thead>
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-success" onclick="Save_dis()" id='btn_confirm'>Confirm</button>
                  </div>
